...
customersRouter.get("/relationship", async function(req, res) {
  try {  
    const { customerId } = req.query
      ...
      SELECT Customer_Relationship.*, Seller.SellerReference, Projects.ProjectName FROM NaturaliStone.Customer_Relationship
      LEFT JOIN Seller ON Seller.SellerID = Customer_Relationship.SellerID
      LEFT JOIN Projects ON Projects.idProjects = Customer_Relationship.ProjectID
      Where Customer_Relationship.CustomerID =${customerId}
      ORDER BY Customer_Relationship.Insert_Date DESC`;
