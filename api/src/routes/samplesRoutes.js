...
samplesRoutes.post('/', function(req, res) {
  ...
  try {
    ...
    // Recorrer el array de valores y construir las cláusulas WHERE
     ...
     const clausula = 
     `Dimension.Material = '${valores.type}' AND Dimension.Type = 'Sample' 
...
// Construir la consulta SELECT
    const sqlSelect = 
    `SELECT Products.ProdNameID, Dimension.Material, Dimension.Type, 
  ...
  if (valoresNoExistentes.length > 0) {
    ...
    prodIds = prodIdPromises
      .then(results => {
        return results
        // Realizar la siguiente consulta con los prodID
        // ...
      })
      .catch(err => {
...
// Utilizar los prodIds fuera del bloque if
  const newProdIds = prodIds
  .then(results => {
    console.log('Array de prodID fuera del bloque if:', results);
    // Realizar otras operaciones con los prodIds
  })
  .catch(err => {
    console.error('Error:', err);
  });

  console.log('soy new prodis', newProdIds)
      ...
      mysqlConnection.query(query_2, function(err, results, field) {
        ...
        const query_3 = `INSERT INTO Samples_Products (SampleID, ProdID, Quantity) Values ?`
        const sampleProductsValue = parsedProducts.map((elem) => [sampleID, elem.prodID, elem.quantity])
        
        mysqlConnection.query(query_3, sampleProductsValue, async function(error, results, fields){
          if (error) {
            console.log('Error in sampleroutes.post /samples ' + error);
            console.log('Retrying ProdSold insert after 0.5 seconds...');
            
            setTimeout(() => {
              mysqlConnection.query(query_3, [sampleProductsValue], async function(error, prodSoldResult, fields) {
                if (error) {
                  console.log('Error in sampleroutes.post /samples ' + error);
