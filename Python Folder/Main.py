    ...
    def on_created(self, event):  
        ...
        if  ('.\\InvoicesReceived\\' in event.src_path and
             '.pdf' in event.src_path  and
                event.src_path not in modifiedEvents and 
                "Parsed Documents" not in event.src_path):
...
for folder in os.listdir('.\\InvoicesReceived')[:-2]:  
    ...
    for file in list_of_files : 
        if file == '.\\InvoicesReceived\\All Attachments Received\\4426 D. Allen Flooring Marmi Tribecca.pdf':
            continue
        elif '.jpg' in file:
            print('\n', file)
            shutil.move(file, 
