...
const CreatedQuotePdf = ({ variables, user, customer, project }) => {
    ...
    return new Promise((resolve) => {
      const checkMappedProducts = () => {
        if (Array.isArray(mappedProducts)) {
          ...
          setTimeout(checkMappedProducts, 100);
        }
      };
  ...
  async function CreateForm() {
    ...
    savePdfOnServer(pdfBytes);
  }

  async function savePdfOnServer(pdfBytes) {
    try {
      const response = await axios.post('/api/save-pdf', pdfBytes, {
        headers: {
